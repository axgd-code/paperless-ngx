...
class DocuSealProvider(BaseIntegrationProvider):
    ...
    def _handle_response(self, response: requests.Response) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"DocuSeal API error: {e.response.status_code} - {e.response.text}")
            raise ConnectionError(f"DocuSeal API returned {e.response.status_code}: {e.response.text}")
        except ValueError:
    ...
    def push_document(
        ...
        try:
            ...
            submission_data = {
                ...
                "send_email": True,
                "subject": params.get("subject", "Please sign this document") if params else "Please sign this document",
            }
            ...
            with open(document_path, "rb") as f:
                files = {
                    "file": (document_path.name, f, document.mime_type or "application/pdf")
                }
            ...
            logger.info(f"Document {document_id} successfully sent to DocuSeal: {submission_id}")
            ...
            logger.error(f"Error pushing document {document_id} to DocuSeal: {str(e)}", exc_info=True)
            return PushResult(
                success=False,
    ...
    def get_status(self, remote_id: str) -> StatusResult:
        ...
        try:
            ...
            logger.error(f"Error getting status for DocuSeal submission {remote_id}: {str(e)}")
            return StatusResult(
    ...
    def get_remote_url(self, remote_id: str) -> Optional[str]:
        """
        ...
        try:
            ...
            logger.warning(f"Could not fetch URL from API, using default: {str(e)}")
    ...
    def delete_remote_document(self, remote_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error deleting DocuSeal submission {remote_id}: {str(e)}")
            return False
    ...
    def validate_credentials(self) -> bool:
        ...
        try:
            ...
            logger.error(f"DocuSeal credential validation failed: {str(e)}")
            return False
